...
class _FunScreenState extends ConsumerState<FunScreen> {
  ...
  Widget build(BuildContext context) {
    final funInterests = ref.watch(funInterestProvider);

    return Scaffold(
      appBar: AppBar(
        title: const Text('재밌는 것'),
      ),
      body: PageView.builder(
        controller: _pageController1,
        itemCount: funInterests.length,
        onPageChanged: (index) {
        ...
        itemBuilder: (context, index) {
          final interest = funInterests[index];
          return Stack(
            children: [
              // 배경 카드
              Center(
                child: Transform.scale(
                  ...
                  child: InterestCard(
                    interest: interest,
                    onTap: () {
                      context.push('/fun/${interest.name}');
                    },
                  ),
                ),
              )
